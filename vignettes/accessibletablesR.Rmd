...
All arguments within the "workbook" function are optional. The default setting for most of the
arguments is NULL (e.g., no cover page will be created). The "workbook" function will create a
workbook named wb in the R global environment. The arguments "subject" and "category" are not used
in the example code above. These are related to fields within the "Info" section of a spreadsheet.
It is possible for the font, font colour and font sizes (different sizes can be specified for
subtitles, titles and the rest of the spreadsheet) to be amended. The default colour is black, the
default font is Arial and the default font sizes are 16 for titles, 14 for subtitles and 12 for the
...
arguments to "Link to contents" or "Link to notes" or "Link to definitions". Although there are six
extralines arguments, each one can be a vector and there is no limit to how many rows can appear
...
If a contents page is desired, the contentstab argument in the "workbook" function must have been
set to "Yes". Run the "contentstable" function after all of the data tables have been converted to
worksheets (i.e., run through the "creatingtables" function). All the arguments in the 
...
autonotes argument in the "workbook" function has been set to "Yes" and that the applictabtext
argument in the "addnote" function is populated.
...
notestab argument in the "workbook" function has been set to "Yes".
...
argument in the "workbook" function has been set to "Yes".
...
been added (see the arguments extrafields and extrafieldsb). Additional links have been provided 
(see the arguments additlinks and addittext). The width of the column has been set to a specific
value (see the colwid_spec argument). The ordering of the fields on the cover page has been
altered (see the argument order). The argument source has been set to a hyperlink (the text 
